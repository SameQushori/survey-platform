...
- Custom flow использует Clerk `signUpIfMissing`: после верного кода отсутствующий пользователь переводится в sign-up без повторного письма. Google SSO завершается на `/sso-callback`, после чего открывается только разрешённый внутренний `/app` route.
...
4. Войти через Google: после возврата должен кратко появиться «Завершаем вход», затем `/app`, без повторного окна авторизации. Embedded browser может блокировать OAuth — это не считается результатом UAT.
5. Зарегистрировать новый email: тот же введённый код должен завершить signup без ошибки `sign_up_if_missing_transfer`; убедиться, что автоматически создано одно пустое личное пространство.
